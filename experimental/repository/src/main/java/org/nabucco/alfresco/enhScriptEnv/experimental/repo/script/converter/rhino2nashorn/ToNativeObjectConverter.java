 ...
 * {@link ScriptNode#addAspect(String, Object) ScriptNode.addAspect} or {@link JscriptWorkflowDefinition#startWorkflow(Object)
 * JscriptWorkflowDefinition.startWorkflow}, which - despite claiming {@code Object} as the type of one parameter - expect a
 * {@code ScriptableObject} from which to extract additional parameters for the operation. Instances of this converter will register
 * themselves for the base type of {@code Object} but exclude themselves unless the exptected result type of
 ...
 * can somehow be converted into a {@link Map}.
 *
 * @author Axel Faust, <a href="http://www.prodyna.com">PRODYNA AG</a>
 */
public class ToNativeObjectConverter implements ValueInstanceConverter, InitializingBean
{
    ...
    public int getForJavaConversionConfidence(final Class<?> valueInstanceClass, final Class<?> expectedClass)
    {
        ...
        if (ScriptableObject.class.equals(expectedClass) || NativeObject.class.equals(expectedClass))
        {
    ...
    {
        final boolean canConvert = (ScriptableObject.class.equals(expectedClass) || NativeObject.class.equals(expectedClass)) && globalDelegate.canConvertValueForJava(value, Map.class);
        return canConvert;
    }
    ...
    {
        final Map<?, ?> propValues = (Map<?, ?>) globalDelegate.convertValueForJava(value, Map.class);

        final Scriptable result;
        final Context cx = Context.enter();
        try
        {
            result = cx.newObject(DUMMY_SCOPE);
            ...
            for (final Entry<?, ?> entry : propValues.entrySet())
            {
                ...
                {
                    result.put(((Number) key).intValue(), result, entry.getValue());
                }
                else
                {
                    result.put(key.toString(), result, entry.getValue());
                }
            }
        }
        finally
