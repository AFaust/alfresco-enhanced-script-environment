...
public abstract class AbstractExecuteBatchFunction implements IdFunctionCall, ScopeContributor, InitializingBean
{
	 ...
	 */
	@Override
	public Object execIdCall(final IdFunctionObject f, final Context cx, final Scriptable scope,
			...
			final Object[] args)
	{
		...
		if (f.hasTag(EXECUTE_BATCH_FUNC_TAG))
		{
			if (f.methodId() == EXECUTE_BATCH_FUNC_ID)
			{
				...
				// optional parameters
				final Pair<Scriptable, Function> beforeProcessCallback = this
						.readCallbackArgument(cx, thisObj, args, 4);
				...
				if (workProviderCallback.getSecond() == null)
				{
					...
					if (workValue != null && workValue != Undefined.instance)
					{
						...
						{
							final Map<Function, Scriptable> savepoint = this.adaptFunctionScopes(processCallback,
							...
							{
								this.restoreFunctionScopes(processCallback, beforeProcessCallback,
										afterProcessCallback, savepoint);
				...
				else
				{
					...
					try
					{
						this.executeBatch(scope, thisObj, workProviderCallback, processCallback, threadCount,
								batchSize,
								beforeProcessCallback, afterProcessCallback);
					} finally
					{
	...
	public void contributeToScope(final Scriptable scope, final boolean trustworthyScript, final boolean mutableScope)
	{
		final IdFunctionObject func = new IdFunctionObject(this, EXECUTE_BATCH_FUNC_TAG, EXECUTE_BATCH_FUNC_ID,
				EXECUTE_BATCH_FUNC_NAME,
				ARITY, scope);
		func.sealObject();

		// export as read-only and undeleteable property of the scope
		ScriptableObject.defineProperty(scope, EXECUTE_BATCH_FUNC_NAME, func, ScriptableObject.PERMANENT
	...
	protected void adaptFunctionScope(final Pair<Scriptable, Function> callback,
			final Map<Function, Scriptable> originalScopeByFunction,
	...
	protected abstract void executeBatch(final Scriptable scope, final Scriptable thisObj,
			final Collection<Object> workItems,
			final Pair<Scriptable, Function> processCallback, final int threadCount, final int batchSize,
			final Pair<Scriptable, Function> beforeProcessCallback,
			final Pair<Scriptable, Function> afterProcessCallback);

	protected abstract void executeBatch(final Scriptable scope, final Scriptable thisObj,
			final Pair<Scriptable, Function> workProviderCallback, final Pair<Scriptable, Function> processCallback,
			final int threadCount,
			final int batchSize, final Pair<Scriptable, Function> beforeProcessCallback,
			final Pair<Scriptable, Function> afterProcessCallback);

	protected Collection<Object> doProvideNextWork(final Scriptable scope,
			...
			final Pair<Scriptable, Function> beforeProcessCallback)
	{
		...
		try
		{
			...
			processScope.setPrototype(this.facadeFactory.toFacadedObject(parentScope, parentScope, null));
			...
			if (beforeProcessCallback.getSecond() != null)
			{
				...
				final Scriptable beforeProcessCallScope = beforeProcessOriginalCallScope == parentScope ? processScope
						: this.facadeFactory.toFacadedObject(parentScope, beforeProcessOriginalCallScope, null);
		...
		try
		{
			if (afterProcessCallback.getSecond() != null)
			{
				...
				final Scriptable afterProcessCallScope = afterProcessOriginalCallScope == parentScope ? processScope
						: this.facadeFactory.toFacadedObject(parentScope, afterProcessOriginalCallScope, null);
	...
	protected Pair<Scriptable, Function> readCallbackArgument(final Context cx, final Scriptable defaultScope,
